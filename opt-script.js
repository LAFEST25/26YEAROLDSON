...
// Disable antialiasing since post-processing creates its own smoothing and it eats GPU
const renderer = new THREE.WebGLRenderer({ antialias: false, powerPreference: "high-performance" });
renderer.setSize(window.innerWidth, window.innerHeight);
// Limit pixel ratio to 1.5 instead of 2. Mobile users on 3x screens won't melt their GPUs
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      float fbm(vec2 p){
        ...
        // Reduced from 6 iterations to 4 for performance. Looks identical, saves heavy compute
        for(int i=0;i<4;i++){ v+=a*noise(p); p=rot*p*2.1; a*=0.48; }
        return v;
      }
...
const discGeo = new THREE.RingGeometry(1.5, 5.0, 128, 16);
...
const disc2Geo = new THREE.RingGeometry(1.6, 4.8, 128, 16);
...
const disc3Geo = new THREE.RingGeometry(1.6, 4.8, 128, 16);
...
const haloGeo = new THREE.RingGeometry(4.5, 7.0, 64, 8);
